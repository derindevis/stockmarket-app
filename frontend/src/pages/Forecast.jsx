...
export default function Forecast() {
  ...
  const handleGenerateForecast = async (e) => {
    ...
    try {
      const res = await api.get(`/forecast/${symbol.trim().toUpperCase()}?days=${days}`)
  ...
  useEffect(() => {
    if (querySymbol) {
      ...
      const timer = setTimeout(() => {
        api.get(`/forecast/${querySymbol.toUpperCase()}?days=${days}`)
          .then(res => setForecastData(res.data))
          .catch(err => {
            console.error(err)
            setError('Failed to generate automatic forecast. Check symbol history.')
          })
      }, 300)
  ...
  const trendColor = 
    forecastData?.trend === 'uptrend' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' :
    forecastData?.trend === 'downtrend' ? 'text-red-400 bg-red-500/10 border-red-500/20' :
    'text-yellow-400 bg-yellow-500/10 border-yellow-500/20'

  return (
    <div className="min-h-screen p-6 space-y-6 bg-[#0a0a0f] text-gray-200">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-wide">🔮 LSTM Price Forecasting</h1>
        <p className="text-gray-400 text-sm mt-1">
          Train a deep learning LSTM Neural Network on historical price closes to forecast future trends.
        </p>
      </div>

      {/* Control panel */}
      <div className="bg-[#12121a] border border-white/[0.08] rounded-xl p-6">
        <form onSubmit={handleGenerateForecast} className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
          <div>
            <label className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Stock Symbol</label>
            <input
              type="text"
        ...
        {loading && (
          <p className="text-xs text-indigo-400 mt-2.5 animate-pulse">
            ⚠️ Note: Training the recurrent neural network dynamically takes approximately 5 to 15 seconds. Please do not close this page.
          </p>
        )}
      </div>
      ...
      {loading && (
        <div className="bg-[#12121a] border border-white/[0.08] rounded-xl p-12 flex flex-col items-center justify-center space-y-4">
          ...
          <div className="text-center space-y-2">
            <h3 className="text-lg font-bold text-white">Training LSTM Neural Network</h3>
            <p className="text-gray-400 text-xs max-w-xs leading-relaxed">
              Standardizing sequence matrices, applying Adam optimizer weights, and computing output standard deviation residuals.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-[#12121a] border border-white/[0.08] rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-white">Projected Price Chart (95% CI)</h2>
              <span className="text-gray-400 text-xs font-semibold">Model: LSTM Recurrent Network</span>
            </div>
          ...
          <div className="bg-[#12121a] border border-white/[0.08] rounded-xl p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div className="bg-[#0a0a0f] border border-white/[0.05] p-4 rounded-xl flex justify-between items-center">
                <div>
                  <span className="text-gray-400 text-xs uppercase tracking-wider">Projected Trend</span>
                  <div className={`text-xl font-bold uppercase mt-1 ${forecastData.trend === 'uptrend' ? 'text-emerald-400' : forecastData.trend === 'downtrend' ? 'text-red-400' : 'text-yellow-400'}`}>
                    {forecastData.trend}
                  </div>
                </div>
                <span className={`px-3 py-1 rounded-full text-xs font-bold border ${trendColor}`}>
                  {forecastData.predicted_change_percent >= 0 ? '+' : ''}{forecastData.predicted_change_percent}%
                </span>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#0a0a0f] border border-white/[0.05] p-4 rounded-xl">
                  <span className="text-gray-400 text-xs uppercase tracking-wider">Current Price</span>
                  <div className="text-lg font-bold text-white mt-1">${forecastData.current_price?.toFixed(2)}</div>
                </div>
                <div className="bg-[#0a0a0f] border border-white/[0.05] p-4 rounded-xl">
                  <span className="text-gray-400 text-xs uppercase tracking-wider">Model Conf.</span>
                  <div className="text-lg font-bold text-white mt-1">{forecastData.confidence}%</div>
                </div>
              </div>

              <div className="bg-[#0a0a0f] border border-white/[0.05] p-4 rounded-xl space-y-2.5">
                <span className="text-gray-400 text-xs uppercase tracking-wider">Neural Network Parameters</span>
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between">
                    <span className="text-gray-500">Architecture:</span>
                    <span className="text-gray-300 font-semibold">Stacked LSTM</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Hidden Layers:</span>
                    <span className="text-gray-300 font-semibold">2 Layers</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Hidden Units:</span>
                    <span className="text-gray-300 font-semibold">64 Units per layer</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-300 font-semibold">Adam (lr=0.001)</span>
                  </div>
                </div>
              </div>
            </div>
