...
export default function AdminUsers() {
  ...
  const handleToggleStatus = async (user) => {
    ...
    try {
      ...
      await api.put(`/admin/users/${user.id}/status`, { is_active: nextStatus })
  ...
  return (
    <div className="min-h-screen p-6 space-y-6 bg-[#0a0a0f] text-gray-200">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-wide">👥 User Administration</h1>
        <p className="text-gray-400 text-sm mt-1">Review user roles, creation timestamps, and manage user statuses.</p>
      </div>
                ...
                {users.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-white/[0.01]">
                      ...
                      <td className="p-4 font-bold text-white text-base">{u.username} {isSelf && <span className="text-xs text-gray-500 font-normal ml-1">(you)</span>}</td>
                      <td className="p-4 text-gray-400">{u.email}</td>
                      <td className="p-4">
                        <span className={`px-2.5 py-0.5 rounded text-xs font-semibold ${u.role === 'admin' ? 'bg-indigo-500/15 text-indigo-400' : 'bg-gray-800 text-gray-400'}`}>
                          {u.role}
                        </span>
                      </td>
                      <td className="p-4">
                        <span className={`px-2 py-0.5 rounded text-xs font-bold ${u.is_active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
