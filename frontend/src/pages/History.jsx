...
export default function History() {
  ...
  const handleClearHistory = async () => {
    if (!window.confirm("Are you sure you want to clear your entire analysis history? This cannot be undone.")) return
    try {
      setError('')
    ...
    <div className="min-h-screen p-6 space-y-6 bg-[#0a0a0f] text-gray-200">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Review your past stock analyses and generated technical reports.</p>
        </div>
        {history.length > 0 && (
          <button
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-gray-400 text-sm max-w-sm mx-auto mb-6">
            You haven't run any stock analyses yet. Head over to the dashboard to search for a stock and calculate its indicators.
          </p>
          <button
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <td className="p-4 font-medium text-white">{item.confidence}%</td>
                        <td className="p-4 text-gray-300">{item.rsi?.toFixed(1) || 'N/A'}</td>
                        <td className="p-4 text-gray-300">{item.macd?.toFixed(3) || 'N/A'}</td>
                      ...
                      {/* Expanded Section */}
                      {isExpanded && (
                        <tr key={`${item.id}-details`} className="bg-[#0c0c14]/40 border-b border-white/[0.05]">
                          <td colSpan="7" className="p-5 space-y-4">
                            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 text-xs">
                              <div>
                                <span className="text-gray-500 uppercase">SMA 20:</span>
                                <p className="font-semibold text-white mt-0.5">${item.moving_average?.toFixed(2) || 'N/A'}</p>
                              </div>
                              <div>
                                <span className="text-gray-500 uppercase">Beta vs SPY:</span>
                                <p className="font-semibold text-white mt-0.5">{item.beta?.toFixed(2) || 'N/A'}</p>
                              </div>
                              <div>
                                <span className="text-gray-500 uppercase">P/E Ratio:</span>
                                <p className="font-semibold text-white mt-0.5">{item.pe_ratio?.toFixed(1) || 'N/A'}</p>
                              </div>
                              <div>
                                <span className="text-gray-500 uppercase">Sharpe Ratio:</span>
                                <p className="font-semibold text-white mt-0.5">{item.sharpe_ratio?.toFixed(2) || 'N/A'}</p>
                              </div>
                              <div>
                                <span className="text-gray-500 uppercase">Volatility:</span>
                                <p className="font-semibold text-white mt-0.5">{item.volatility?.toFixed(1) || 'N/A'}%</p>
                              </div>
                            </div>
                            
                            <div className="bg-[#0a0a0f] border border-white/[0.05] p-4 rounded-lg">
                              <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-1.5">AI Analysis Explanation</h4>
                              <p className="text-xs text-gray-400 leading-relaxed font-normal">{item.ai_explanation}</p>
                            </div>
                          </td>
                        </tr>
                      )}
