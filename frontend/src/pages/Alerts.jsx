...
export default function Alerts() {
  ...
  return (
    <div className="min-h-screen p-6 space-y-6 bg-[#0a0a0f] text-gray-200">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-wide">🔔 Indicator Alerts</h1>
        <p className="text-gray-400 text-sm mt-1">Configure automated notifications based on key technical indicator values.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#12121a] border border-white/[0.08] rounded-xl p-6 h-fit space-y-4">
          ...
          <form onSubmit={handleCreateAlert} className="space-y-4">
            ...
            <div>
              <label className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Indicator Type</label>
              <select
                value={alertType}
            ...
            <div>
              <label className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Threshold Value</label>
              <input
                type="number"
                step="any"
                placeholder={alertType === 'rsi_threshold' ? 'e.g. 30' : alertType === 'volatility_spike' ? 'e.g. 45' : 'e.g. 1'}
                value={threshold}
        ...
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-lg font-bold text-white">Active Alerts ({alerts.length})</h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-24 bg-[#12121a] border border-white/[0.08] rounded-xl p-8">
              ...
              <p className="text-gray-400 text-sm max-w-sm mx-auto">
                No active alert configurations found. Set up alerts on the left to monitor metric thresholds.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {alerts.map((item, index) => {
                const label = 
                  item.alert_type === 'rsi_threshold' ? 'RSI Threshold' :
                  item.alert_type === 'volatility_spike' ? 'Volatility Spike' :
                  'Signal Change'
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-bold text-white tracking-wide">{item.symbol}</span>
                        <span className={`w-2 h-2 rounded-full ${item.is_triggered ? 'bg-emerald-400 animate-pulse' : 'bg-yellow-400'}`}></span>
                      </div>
                      <p className="text-gray-400 text-xs mt-1 font-semibold">{label}</p>
                      <div className="text-xs text-gray-500 mt-4">
                        Threshold: <span className="font-semibold text-white">{item.threshold}</span>
                      </div>
                      <div className="text-[10px] text-gray-500 uppercase mt-0.5">
                        Status: <span className={item.is_triggered ? 'text-emerald-400' : 'text-yellow-400'}>
